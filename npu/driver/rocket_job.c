...
static struct dma_fence *rocket_job_run(struct drm_sched_job *sched_job)
{
	...
	scoped_guard(mutex, &core->job_lock) {
		...
		/* busy prozor se otvara ovde — 1. od 3 mesta koja diraju in_flight_job */
...
rocket_reset(struct rocket_core *core, struct drm_sched_job *bad)
{
	...
	scoped_guard(mutex, &core->job_lock) {
		...
		core->in_flight_job = NULL;
		/*
		 * 3. od 3 mesta — ono koje se najlakse zaboravi. Bez ovoga bi
		 * jedan reset ostavio brojac "zauvek zauzet", governor bi video
		 * 100% i zakucao se na maksimalan takt do ponovnog ucitavanja.
		 */
		rocket_devfreq_record_idle(core);
	}
