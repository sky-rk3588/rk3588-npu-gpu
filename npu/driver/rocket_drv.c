...
static int rocket_probe(struct platform_device *pdev)
{
	...
	if (!ret) {
		if (core == 0)
			rocket_devfreq_init(&rdev->cores[core]);
		else
			rocket_devfreq_core_added(&rdev->cores[core]);
	}
	if (ret) {
...
static void rocket_remove(struct platform_device *pdev)
{
	...
	rocket_devfreq_core_removed(&rdev->cores[core]);
	rocket_devfreq_fini(&rdev->cores[core]);
...
static void rocket_platform_shutdown(struct platform_device *pdev)
{
	int core = find_core_for_dev(&pdev->dev);

	if (core >= 0)
		rocket_devfreq_shutdown(&rdev->cores[core]);
}
...
static int rocket_device_runtime_suspend(struct device *dev)
{
	...
	/* jos smo pod naponom — vrati bezbedan takt PRE gasenja domena */
	err = rocket_devfreq_suspend(&rdev->cores[core]);
	if (err)
		return err;
...
static struct platform_driver rocket_driver = {
	...
	.shutdown = rocket_platform_shutdown,
	.driver	 = {
		.name = "rocket",
